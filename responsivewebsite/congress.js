...
app.controller('statedata', function ($scope, $http) {
    ...
    $http({method: "GET", url: "index.php", params: {"states":$scope.selectedState.abb}})
   .then(
       function(response){
         // success callback
...
app.controller('listdata', function ($scope, $http) {
   ...
   .then(
    ...
    $scope.viewDetails = function(para) {
        ...
        $http({method: "GET", url: "index.php", params: {"bio_guide":$scope.para, "type": $scope.type}})
            .then(function(result) {
                $scope.details = result.data.results[0];
            $scope.bio_guide_id=$scope.details.bioguide_id;
            $scope.title = $scope.details.title;
            $scope.last_name = $scope.details.last_name;
            $scope.first_name = $scope.details.first_name;
            $scope.oc_email = $scope.details.oc_email;
            $scope.chamber = $scope.details.chamber;
            $scope.party=$scope.details.party;
            $scope.phone=$scope.details.phone;
            $scope.office=$scope.details.office;
            $scope.state_name=$scope.details.state_name;
            $scope.fax=$scope.details.fax;
            $scope.twitter_id=$scope.details.twitter_id;
            $scope.facebook_id=$scope.details.facebook_id;
            $scope.website=$scope.details.website;
            ...
            var now_dt = new Date();
            $scope.prgrs= Math.round(((now_dt-st_dt)/(en_dt-st_dt)*100));
            ...
            $("#star_icon_biof").addClass("glyphicon-star");
             $("#star_icon_biof").css("color","yellow");   
                ...
                if(typeof(Storage) !== "undefined") {
                    ...
                    array.push({ 'state': $scope.details.state_name, 'email': $scope.details.oc_email, 'bioguide_id': $scope.bio_guide_id, 'party': $scope.party, 'last_name': $scope.details.last_name, 'first_name': $scope.details.first_name, 'chamber': $scope.details.chamber});
...
app.controller('housedata', function ($scope, $http) {
    $scope.houseusers = [];
    $scope.type="House";
    $http({method: "GET", url: "index.php", params: {"type":$scope.type}})
   .then(
       function(response){
         // success callback
...
app.controller('billinfo', function ($scope, $http) {
  ...
  $scope.viewBillDetails = function(para){
    ...
    $http({method: "GET", url: "index.php", params: {"billid":para, "type":$scope.type}})
   .then(
       function(response){
         // success callback
           $scope.billsDetails = response.data.results[0];
           /*$scope.introduced_on = $scope.billsDetails.introduced_on;*/
           var intro_date = new Date($scope.billsDetails.introduced_on);
             var months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
            $scope.introduced_on=(months[intro_date.getMonth()]) + ' ' + (intro_date.getDate()+1) + ', ' +  intro_date.getFullYear();
        ...
        if($("#star_icon_bills").hasClass("glyphicon-star-empty")){
            ...
            $("#star_icon_billsf").css("color","yellow");   
                ...
                if(typeof(Storage) !== "undefined") {
                    ...
                    //array=JSON.stringify(array);
                    billarray.push({ 'bill_id': $scope.billsDetails.bill_id, 'bill_type': $scope.billsDetails.bill_type, 'title': $scope.billsDetails.official_title, 'chamber': $scope.billsDetails.chamber, 'introduced_on': $scope.billsDetails.introduced_on, 'stitle': $scope.billsDetails.sponsor.title, 'slast_name': $scope.billsDetails.sponsor.last_name, 'sfirst_name': $scope.billsDetails.sponsor.first_name});
  ...
  $scope.getFavBills = function() {
   ...
   $scope.removeBill= function(para){
    ...
    $http({method: "GET", url: "index.php", params: {"committeeid":para, "type":$scope.type}})
   .then(
       function(response){
         // success callback
